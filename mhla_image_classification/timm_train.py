...
from models import timm_registers
from models import mala_model
...
# 添加profiler相关参数
group.add_argument('--enable-profiler', action='store_true', default=False,
                   help='启用torch.profiler进行性能分析 (默认: False)')
group.add_argument('--profiler-epochs', type=int, default=1, metavar='N',
                   help='对多少个epoch进行性能分析 (默认: 1)')
group.add_argument('--profiler-start-epoch', type=int, default=1, metavar='N',
                   help='从第几个epoch开始进行性能分析 (默认: 1)')
group.add_argument('--profiler-warmup-steps', type=int, default=2, metavar='N',
                   help='profiler预热步数 (默认: 2)')
group.add_argument('--profiler-active-steps', type=int, default=3, metavar='N',
                   help='profiler活跃分析步数 (默认: 3)')
group.add_argument('--profiler-wait-steps', type=int, default=1, metavar='N',
                   help='profiler等待步数 (默认: 1)')
group.add_argument('--profiler-repeat', type=int, default=1, metavar='N',
                   help='profiler重复分析轮次 (默认: 1)')
...
def main():
    ...
    # 初始化profiler配置
    profiler_ctx = None
    if args.enable_profiler and utils.is_primary(args):
        try:
            # 创建TensorBoard日志目录
            tensorboard_dir = os.path.join(output_dir if output_dir else '.', 'profiler_logs')
            os.makedirs(tensorboard_dir, exist_ok=True)
            
            _logger.info(f'启用profiler，将从第{args.profiler_start_epoch}个epoch开始，持续{args.profiler_epochs}个epoch')
            _logger.info(f'TensorBoard profiler日志保存到: {tensorboard_dir}')
            ...
            _logger.warning(f'无法导入torch.profiler模块: {e}. Profiler功能将被禁用.')
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            # 确定是否需要在此epoch启用profiler
            current_profiler = None
            if (args.enable_profiler and utils.is_primary(args) and 
                epoch >= args.profiler_start_epoch and 
                epoch < args.profiler_start_epoch + args.profiler_epochs):
                
                # 为当前epoch创建profiler context
                current_profiler = profile(
                    activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                    schedule=profiler_schedule,
                    on_trace_ready=torch.profiler.tensorboard_trace_handler(
                        os.path.join(tensorboard_dir, f'epoch_{epoch}')
                    ),
                    record_shapes=True,
                    profile_memory=True,
                    with_stack=True
                )
                current_profiler.__enter__()
                _logger.info(f'在第{epoch}个epoch启用profiler')
            ...
            train_metrics = train_one_epoch(
                ...
                profiler=current_profiler,  # 传递profiler实例
            )

            # 结束profiler context
            if current_profiler is not None:
                current_profiler.__exit__(None, None, None)
                _logger.info(f'第{epoch}个epoch的profiler分析完成')
...
def print_model_acts(model):
    for i, block in enumerate(model.blocks):
        print(f"  Number {i} block:")
        print(f"  min_act: {block.attn.min_act}")
        print(f"  max_act: {block.attn.max_act}")
        print(f"  has_inf: {block.attn.has_inf}")
        print(f"  has_nan: {block.attn.has_nan}")
...
def train_one_epoch(
        ...
        profiler=None,  # 添加profiler参数
):
    # print_model_acts(model.module)
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                # 添加profiler记录函数，用于标记前向传播过程
                with record_function("forward_pass") if profiler else suppress():
                    output = model(input)
        ...
        def _backward(_loss):
            # 添加profiler记录函数，用于标记反向传播过程
            with record_function("backward_pass") if profiler else suppress():
        ...
        # 添加profiler step调用，用于profiler schedule管理
